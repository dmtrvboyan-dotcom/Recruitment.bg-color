    ...
    <div className="border border-[var(--color-border)] rounded-2xl overflow-hidden bg-white mb-4 last:mb-0 transition-all duration-300">
      ...
      >
        <div className="flex items-center gap-4">
          <div className="w-8 h-8 rounded-full bg-[var(--color-accent-primary)]/10 flex-shrink-0 flex items-center justify-center">
            <HelpCircle className="w-5 h-5 text-[var(--color-accent-primary)]" />
          </div>
          <h3 className="text-xl font-semibold text-[var(--color-text-primary)] pr-8 leading-tight">
            {faq.q}
          </h3>
        </div>
        <ChevronDown
          className={`w-6 h-6 text-[var(--color-accent-primary)] transition-transform duration-500 shrink-0 hover:text-[var(--color-accent-primary)]/70 ${
            isOpen ? "rotate-180" : ""
          }`}
        />
      </button>
      ...
      >
        <div className="px-6 pb-8 pl-17 text-lg text-[var(--color-muted-foreground)] leading-relaxed">
          {faq.a}
        </div>
      </div>
    </div>
